...
def test_model_compare_default_excludes_extra(mock_client, storage):
    config = ExperimentConfig(name="model_compare", prompt="test")
    exp = ModelCompareExperiment(mock_client, storage, config)
    results = exp.run()
    models_used = [r.model for r in results]
    assert "moonshotai/kimi-k2.5" not in models_used
    assert len(results) == 2

def test_model_compare_includes_kimi_when_opt_in(mock_client, storage):
    config = ExperimentConfig(name="model_compare", prompt="test")
    exp = ModelCompareExperiment(mock_client, storage, config, include_opus=True)
    results = exp.run()
    assert len(results) == 3
    assert any(r.model == "moonshotai/kimi-k2.5" for r in results)
